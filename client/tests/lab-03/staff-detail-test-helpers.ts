...
export function installStaffDetailFetch(options: {
  ...
  const mock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.pathname === `/api/staff/tickets/${ticket.id}`) {
      return options.detailResponse?.() ?? jsonResponse(ticket);
    }
    ...
    if (url.pathname.endsWith("/attachments")) return jsonResponse({ items: [{
    ...
    if (url.pathname.endsWith("/comments")) return jsonResponse({
    ...
    if (url.pathname.endsWith("/notes")) return jsonResponse({
