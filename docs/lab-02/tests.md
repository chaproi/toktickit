...
My Tickets API and UI tests shall use deterministic, isolated data with uniquely identifiable records and shall clean up every record they create. Existing Tickets must not affect asserted totals, ordering, empty states, or no-results states.
...
| `UNIT-04` | Unit                   | BR-32–BR-42, AC-20–AC-23                       | Search, filter, sort, page, page-size, unknown-parameter, and repeated-parameter parsing                    | Valid parameters produce a normalized query; stable secondary and priority ordering are defined; invalid, unknown, or repeated parameters are rejected | `server/tests/lab-02/ticket-query.unit.test.ts`                                                           | Planned |
...
| `API-05`  | API                    | FR-13, FR-26, AC-18, AC-19, AC-26              | My Tickets ownership isolation, requester switching, safe failures, and deterministic test-data isolation  | Returns only the selected Requester’s Tickets, never leaks another Requester’s data, and remains independent of existing Tickets | `server/tests/lab-02/my-tickets.api.test.ts`                                                              | Planned |
| `API-06`  | API                    | FR-14–FR-18, AC-20–AC-25                       | Applied search and filters, all approved sorting rules, out-of-range pagination, zero-result metadata, empty responses, and query rejection | Returns correctly ordered items; accurate `totalPages: 0` and out-of-range metadata; successful empty results for unmatched positive reference IDs; and `400 INVALID_QUERY` for unknown or repeated parameters | `server/tests/lab-02/my-tickets.api.test.ts`                                                              | Planned |
...
| `UI-05`   | UI component           | FR-13–FR-19, AC-18–AC-26                       | My Tickets Ticket Date display, Ticket Detail link target, explicit Apply and Clear behavior, immediate sort/direction/page-size behavior, out-of-range correction, screen classification, and Requester-safe loading | Correct controls, items, links, metadata, empty, no-results, failure, and final-page states render with accessible labels, keyboard-reachable controls, non-color badge text, and failure-state semantics | `client/tests/lab-02/MyTickets.test.tsx`                                                                  | Planned |
...
* [ ] Search and filters use an explicit Apply action; sort field, sort direction, and page-size changes apply immediately; each applicable change resets to page 1.
* [ ] Ticket rows and cards display Ticket Date and all other required summary information.
* [ ] Ticket Number or row links target `/tickets/:ticketId` without requiring Ticket Detail content in Issue #17.
* [ ] Zero items without applied search or filters show the empty state; zero items with applied search or filters show no-results; sorting alone does not change that classification.
* [ ] An out-of-range page automatically requests the final valid page only when `totalPages` is greater than zero; zero total items do not refetch and hide pagination.
* [ ] Long summaries do not break the layout.
* [ ] Ownership-safe results are maintained when the selected Requester changes.
* [ ] Controls have accessible labels and are keyboard reachable; Priority and Status badges include text; API failures use appropriate failure semantics.
...
* Issue #17 verifies My Tickets-specific accessible labels, keyboard-reachable controls, non-color badge text, and failure-state semantics in `UI-05`. Full cross-screen accessibility and end-to-end evidence remain deferred to a later Lab 2 verification Issue.
